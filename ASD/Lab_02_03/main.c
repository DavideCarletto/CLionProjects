...
typedef enum {
    stampa, o_data, o_codice, o_partenza, o_arrivo, r_partenza,r_leggiFile, r_fine, r_err
}comando;

void leggiFile();
void menuTratte(s_tratta *elencoTratte, int dim, s_tratta ***elencoPuntatori);
int leggiComando();
void stampaTratte(s_tratta *elencoTratte, int dim);
void stampaTratteP(s_tratta **elencoPuntatoriT,int dim);
void ordina(s_tratta **elencoTratteP, int dim, int orderBy);
...
void dealloca(s_tratta *elencoTratte, s_tratta ***elencoPuntatori, s_tratta *tratteTrovate, char *partenza);
...
void leggiFile(char *nome){
    FILE *fpin;
    s_tratta *elencoTratte=NULL, ***elencoPuntatori=NULL;
    ...
    {
        fscanf(fpin,"%d",&dim);
        elencoTratte=(s_tratta *)malloc(dim* sizeof(s_tratta));
        elencoPuntatori=(s_tratta ***) malloc(MAX_T* sizeof(struct tratte **));

        for(i=0;i<MAX_T;i++){
            (elencoPuntatori)[i]=(struct tratte **) calloc(dim, sizeof(struct tratte *));
        }
        i = 0;
        while (!feof(fpin))
        {
            fscanf(fpin, "%s %s %s %s %s %s %d", elencoTratte[i].codice, elencoTratte[i].partenza, elencoTratte[i].destinazione, elencoTratte[i].data, elencoTratte[i].oraPartenza, elencoTratte[i].oraArrivo, &elencoTratte[i].ritardo);
            i++;
        }
        for (j = 0; j < dim; j++) {
            elencoPuntatori[0][j] = elencoPuntatori[1][j] = elencoPuntatori[2][j] = elencoPuntatori[3][j] = elencoPuntatori[4][j] = &elencoTratte[j];
            strcpy(elencoTratte[j].stringhe[0],elencoTratte[j].data);
            strcpy(elencoTratte[j].stringhe[1], elencoTratte[j].codice);
            strcpy(elencoTratte[j].stringhe[2], elencoTratte[j].partenza);
            strcpy(elencoTratte[j].stringhe[3], elencoTratte[j].destinazione);
        }
...
void menuTratte(s_tratta *elencoTratte, int dim, s_tratta ***elencoPuntatori){
    ...
    s_tratta *tratteTrovate = (s_tratta*) malloc(dim*sizeof (s_tratta));
    char *partenza = (char*)malloc(sizeof(char) * (MAX_S   + 1)), *nomeFile = (char *) malloc(sizeof (char )*(MAX_S+1));
    ...
    while(continua){
        printf("CMD 'stampa': stampa le tratte\n"
               ...
               "CMD 'r_leggiFile': lettura da nuovo file\n"
        ...
        switch (comando) {
            ...
            case o_data: case o_codice: case o_partenza: case o_arrivo: //nel caso uno di questi comandi si esegue la stessa funzione
            ...
            case r_partenza:
                ...
                break;
            case r_leggiFile:
                printf("Inserire il nome del nuovo file da cui leggere: ");
                scanf("%s", nomeFile);
                dealloca(elencoTratte, elencoPuntatori, tratteTrovate, partenza);
                leggiFile(nomeFile);
                break;
            case r_fine:
                printf("Finito");
...
int leggiComando(){
    char comando[MAX_S];
    char tabella[8][MAX_S] = {"stampa", "o_data", "o_codice", "o_partenza", "o_arrivo", "r_partenza", "r_leggiFile", "fine"};
...
void dealloca(s_tratta *elencoTratte, s_tratta ***elencoPuntatori, s_tratta *tratteTrovate, char *partenza){
    ...
    for(int i=0; i<MAX_T; i++){
        free((elencoPuntatori)[i]);
    }
    free(elencoPuntatori);
}